...
function isHealthRow(value: unknown): value is HealthRow {
  ...
  return typeof row.health === "string" && HEALTH_STATES.has(row.health) && validDate(row.last_checked_at);
}
    ...
    <section className="industrial-border p-5 md:p-6" aria-labelledby="source-health-heading">
      <div className="flex flex-wrap items-baseline justify-between gap-3 border-b border-white/15 pb-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-text-2">See whether official sources were checked successfully or need attention.</p>
        </div>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-px bg-white/15 md:grid-cols-4">
        <div className="bg-bg-0 p-4"><dt className="micro-label text-muted">Current</dt><dd className="mt-2 font-mono text-2xl font-black tabular-nums text-emerald-300">{rows ? counts.healthy : "..."}</dd></div>
